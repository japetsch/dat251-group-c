...
$: slotsThisMonth = filteredSlots.filter((slot) => {
  ...
  $: selectedSlot =
    safeSlots.find((slot) => String(getSlotId(slot)) === String(selectedSlotId)) ?? null;
</script>
...
<div class="mx-auto max-w-7xl px-6 py-10 md:px-8">
  <div class="mb-8">
    <h1 class="text-4xl font-bold tracking-tight text-slate-900">Bestill ny time</h1>
  ...
  {#if data.error}
    <div class="rounded-[28px] border border-red-200 bg-red-50 px-5 py-4 text-red-700 shadow-sm">
      {data.error}
    </div>
  {:else}
    <div class="space-y-6">
      <section class="rounded-[32px] bg-white p-6 shadow-[0_16px_50px_rgba(15,23,42,0.06)] ring-1 ring-black/5 md:p-8">
        <div class="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
          <div class="max-w-xl">
            <h2 class="text-2xl font-semibold text-slate-900">Tilgjengelige timer</h2>
            <p class="mt-2 text-sm text-slate-500">
              Filtrer etter blodbank og bla mellom ukene for å se ledige tidspunkt.
            </p>
          </div>

          <div class="grid gap-4 sm:grid-cols-[minmax(220px,320px)_auto]">

            <div class="inline-flex rounded-full bg-[#f3eded] p-1 ring-1 ring-[#eadede]">
              <button
                type="button"
            ...
            <label class="block">
              <span class="mb-2 block text-sm font-medium text-slate-700">Velg blodbank</span>
              <select
      ...
      <section class="rounded-[32px] bg-white p-4 shadow-[0_16px_50px_rgba(15,23,42,0.06)] ring-1 ring-black/5 md:p-6">
        <div class="mb-4 flex items-center justify-between px-2">
          <div>
            ...
            <p class="mt-1 text-lg font-semibold text-slate-900">
              {#if viewMode === "week"}
                {formatDateLabel(weekDays[0].toISOString())} – {formatDateLabel(weekDays[6].toISOString())}
              {:else}
          ...
          <span class="rounded-full bg-red-50 px-3 py-1 text-sm font-medium text-red-600">
            {viewMode === "week" ? slotsThisWeek.length : slotsThisMonth.length} ledige tider
          </span>
        </div>
  ...
  <div class="grid gap-4 lg:grid-cols-7">
    {#each groupedByDay as { day, items }, index}
      <div class="min-h-[280px] rounded-[28px] bg-[#fcfbfb] p-4 ring-1 ring-[#efe7e7]">
        <div class="mb-4 border-b border-[#efe7e7] pb-3">
          <p class="text-sm font-medium uppercase tracking-[0.16em] text-slate-400">
            {weekdayNames[index]}
          </p>
        ...
        {#if items.length === 0}
          <div class="rounded-2xl bg-white px-4 py-5 text-sm text-slate-400 ring-1 ring-[#f2ebeb]">
            Ingen ledige tider
          </div>
        {:else}
          <div class="space-y-3">
            {#each items as slot}
              <button
                type="button"
                class:selected={String(selectedSlotId) === String(getSlotId(slot))}
        ...
        <div class="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
            ...
            {#if selectedSlot}
              <div class="mt-3 space-y-1 text-slate-600">
                ...
                <p>
                  ...
                  {formatDateLabel(getSlotTime(selectedSlot))} kl. {formatTime(getSlotTime(selectedSlot))}
                </p>
              </div>
            {:else}
          ...
          {#if selectedSlot}
            ...
            <form method="POST" action="?/book" class="shrink-0">
              ...
              <input type="hidden" name="bookingSlotId" value={String(getSlotId(selectedSlot))} />
              <button
                type="submit"
