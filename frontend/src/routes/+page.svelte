...
<div class="min-h-screen bg-[#f5f2f1] text-black">
    <section class="relative flex min-h-screen items-center justify-center overflow-hidden">
        <div class="absolute inset-0 bg-cover bg-bottom bg-no-repeat opacity-100"
        ...
        <div class="relative z-10 inline-block rounded-[32px] border border-black/10 bg-white px-[10rem] py-[6rem] text-center shadow-[0_4px_18px_rgba(0,0,0,0.05)]">
            <div class="flex flex-col items-center gap-14">
                <img src="/tmp_logo.svg" alt="Norges Blodbank logo" class="mx-auto h-40 w-40"/>
                ...
                <div class="mt-8 flex justify-center gap-6">
                    <Buttons variant="primary" class="px-12 py-8 !text-3xl" onclick={() => (showEligibility = true)}>Sjekk om du kan donere</Buttons>
                    <Buttons variant="secondary" class="px-12 py-8 !text-3xl" onclick={scrollToInfo}>Les mer</Buttons>
                </div>
            </div>
        </div>
    </section>
        ...
        <div class="mx-auto max-w-6xl space-y-12">
            <div class="space-y-4 text-center">
                ...
                <p class="mx-auto max-w-3xl text-lg leading-8 text-black/65">
                    Bloddonasjon er en av de viktigste måtene du kan hjelpe andre på.
                    Hvor donasjon kan redde opptil tre liv.
                </p>
            </div>
            ...
            <div class="flex flex-wrap justify-center gap-6">
                {#each stats as stat}
                    <Cards orientation="vertical" tone="white" class="text-center">
                        <div class="text-4xl font-semibold tracking-tight">{stat.value}</div>
            ...
            <div class="flex flex-wrap justify-center gap-6">
                ...
                <Cards title="Hva skjer under donasjonen?" orientation="vertical" tone="white">
                    <ul class="space-y-2">
            ...
            <div class="flex justify-center">
                <Cards title="Trygt og sikkert" orientation="horizontal" tone="red">
                    <p>Alle donasjoner gjennomføres med sterilt engangsutstyr, og du blir fulgt opp av helsepersonell gjennom hele prosessen.</p>
                </Cards>
            </div>
    ...
    {#if showEligibility}
        <Button type="button" class="fixed" aria-label="Lukk donor-sjekk" onclick={() => (showEligibility = false)}></Button>
        <div class="fixed inset-0 z-50 flex items-center justify-center p-6">
            <Cards orientation="horizontal" tone="white" class="w-full max-w-2xl">
                <div class="space-y-6">
                    <div class="flex items-start justify-between gap-4">
                        <div>
                            ...
                            <p class="mt-2 text-black/70">
                                Dette er en enkel veiledning og erstatter ikke vurdering fra blodbanken.
                            </p>
                        </div>
                        <Buttons variant="secondary" onclick={() => (showEligibility = false)}>Lukk</Buttons>
                    </div>
                    <div class="space-y-4">
                        <div class="rounded-[20px] border border-black/10 bg-[#faf7f6] px-6 py-5">
                            <p class="mb-3 text-lg font-medium">Er du mellom 18 og 70 år?</p>
                            <div class="flex gap-3">
                                <Button type="button" class={`rounded-full border px-5 py-2.5 text-sm font-medium transition ${
                                    ageOk === true
                        ...
                        <div class="rounded-[20px] border border-black/10 bg-[#faf7f6] px-6 py-5">
                            <p class="mb-3 text-lg font-medium">Føler du deg frisk og i god helse i dag?</p>
                            <div class="flex gap-3">
                                <Button type="button" class={`rounded-full border px-5 py-2.5 text-sm font-medium transition ${
                                    healthyOk === true
                                        ? "border-black/15 bg-[#d9d2cf] text-black hover:bg-[#f3eeed]"
                        ...
                        <div class="rounded-[20px] border border-black/10 bg-[#faf7f6] px-6 py-5">
                            <p class="mb-3 text-lg font-medium">Har det gått lenge nok siden sist du donerte blod?</p>
                            <div class="flex gap-3">
                                <Button type="button" class={`rounded-full border px-5 py-2.5 text-sm font-medium transition ${
                                    recentDonationOk === true
                                        ? "border-black/15 bg-[#d9d2cf] text-black hover:bg-[#f3eeed]"
                    ...
                    {#if allAnswered}
                        <div class={`rounded-[20px] border p-5 ${eligible ? "border-green-200 bg-green-50" : "border-red-200 bg-red-50"}`}>
                            {#if eligible}
                                <p class="mb-3 text-lg font-medium">Du kan sannsynligvis donere blod.</p>
                                <p class="mt-2 text-black/70">
                                    Neste steg er å bestille time eller kontakte blodbanken for endelig vurdering.
                                </p>
                            {:else}
                                <p class="mb-3 text-lg font-medium">Du bør kontakte blodbanken før du bestiller time.</p>
                                <p class="mt-2 text-black/70">
                                    Ett ellere flere svar tyder på at du kanskje ikke kan donere blod akkurat nå.
                                </p>
                            {/if}
                        </div>
                    {/if}
                    ...
                    <div class="flex justify-end">
                        <Buttons variant="secondary" onclick={resetEligibility}>Nullstill</Buttons>
                    </div>
                </div>
            </Cards>
        </div>
